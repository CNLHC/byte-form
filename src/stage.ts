    ...
    switch (va.type) {
        ...
        case "compare":
            switch (va.op){
                ...
                case "gt": validateStatus = !!value ? (value > va.value) : true; break;
                case "gte": validateStatus = !!value ? (value >= va.value) : true; break;
            }
            e.fieldStore[key].validate = e.fieldStore[key].validate && validateStatus
            break;
        default:
...
export const pipeSendBack: (cb: (e: any) => void) => PiplineStage = (cb: (e: any) => void) => (e) => {
    cb(
        Object.keys(e.fieldStore).reduce((acc, cur) => ({
            ...acc,
            [cur]: {
...
export const pipeMutation: (fs: ByteFormFieldState) => PiplineStage = (fs) => (e) => {
    ...
    for (const schema of schemas) if (!!schema) {
        ...
        try {
            ...
            shouldFieldMutated = eval(schema.condition.replace(new RegExp(`{{(.*?)}}`, "g"), `(e.fieldStore[fs.key].value)`))
            const keysMutated = schema.mutation.map(e => e.key)
            if (shouldFieldMutated)
                for (const k of keysMutated) {
                    !!e.metaSource[k] ? e.metaSource[k] = { ...e.metaCache[k], ...schema.mutation.find(e => e.key === k) } : {};
                }
        } catch (e) {
...
export const pipeBindValue: (fs: ByteFormFieldState, value: ByteFormFieldState['value']) => PiplineStage = (fs, value) =>
    (e) => {
        if (!e.bindToExternal)
            e.fieldStore[fs.key].value = value
        return e
    }
...
export const pipeRefreshMetaCache: (fms: FieldMetaList) => PiplineStage = (fms) => (e) => {
    ...
    e.fieldStore = Object.keys(e.fieldStore).filter(a => newKeys.includes(a)).reduce((acc, cur) => ({ ...acc, [cur]: (e.fieldStore[cur]) }), {})

    return e
}


export const pipeBindToExternal: (fms: FieldMetaList, value?: { [key: string]: any }) => PiplineStage = (fms, value) => (e) => {
    if (!value) {
    ...
    for (const key of valueKeys) {
        if (!e.fieldStore[key])
            ...
            e.fieldStore[key].value = value[key]
    }
    return e;
}
