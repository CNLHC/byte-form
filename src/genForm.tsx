...
const SwitchField = (props: { fs: ByteFormFieldState; cb?: (e: any) => void; ef?: IExtraField; vff?: boolean }) => {
    ...
    useEffect(() => {
        dispatch(
            action.pipeline(
                composePipeline(pipeBindValue(props.fs, undefined), pipeSendBack((e: any) => !!cb && cb(e))),
                'init key',
            ),
        );
    }, []);
    ...
    switch (type) {
        ...
        default:
            if (ef && ef[type]) {
                ...
                return <Component onChange={(e: any) => handleChange(e)} value={fs.value} />;
            }
            return null;
    }
};
...
const WrappedField = forwardRef(
    (props: { fm: AllFieldMetaUnion; cb?: (e: any) => void; ef?: IExtraField; vff?: boolean }) => {
        ...
        useEffect(() => {
            dispatch(
                action.pipeline(
                    composePipeline(pipeInit(fm), pipeSendBack((e: any) => !!cb && cb(e))),
...
const GenedForm = (props: IConnectorProps & innerProps) => {
    ...
    return (
        <Form wrapperCol={props.wrapperCol} labelCol={props.labelCol} labelAlign={props.labelAlign}>
            {items}
        </Form>
    );
};
